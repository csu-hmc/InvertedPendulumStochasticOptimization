function [Fsee,dFseedx,f,dfdx,dfdxdot,dfdu] = getMusDyns_asat(x,xdot,u,params)
...
a_satup = 1-1/2*((1-a)+sqrt((1-a).^2+params.epsilon^2));
a_sat = 1/2*(a_satup+sqrt(a_satup.^2+params.epsilon^2));
da_satupda = 1/2*(1+(1-a)./sqrt((1-a).^2+params.epsilon^2));
da_satda_satup = 1/2*(1+a_satup./sqrt(a_satup.^2+params.epsilon^2));
da_satda = da_satda_satup.*da_satupda;
...
Flce = a_sat.*fmax.*flce.*glce+lcedot.*fmax/1000; % damping force
dFlcedx = [zeros(nmus,ndof*2) diag(fmax.*flce.*glce.*da_satda) diag(a_sat.*fmax.*glce.*dflcedlce)];
dFlcedxdot = [zeros(nmus,ndof*2+nmus) diag(a_sat.*fmax.*flce.*dglcedlcedot+fmax/1000)];
